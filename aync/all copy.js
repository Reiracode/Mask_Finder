...
// await 顧名思義就是「等待」，它會確保一個 promise 物件都解決(resolve) 或出錯(reject) 後才會進行下一步，當 async function 的內容全都結束後，會返回一個 promise，
// 這表示後方可以使用.then語法來做連接，基本的程式長相就像下面這樣：
// FETCH 取值 非同步 等待取值
//}
...
Promise.all([request(), request1()]).then(result => {
...
const request2 = async () => {
    var newposition = await getYourPosition()
    return newposition;
}
...
var astring;
(async function () {
    astring = await getMaskInfo();
    return astring;
}())
// var ss = await getMaskInfo();
async function add1() {
    const a = await getMaskInfo();
    return a;
}
add1().then(v => {
